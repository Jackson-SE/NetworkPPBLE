...
#'           initial = rep( 1, 2 )
#'           )
CL_LOOCV <- function( CF = GaussianCF,
                      CF_para = list(),
                      para_to_optim,
                      x,
                      fx,
  ...
  LOOCV_accuracy <- function( logz ){
    ...
    E_fxi <- matrix( NA, nrow = n, ncol = k )

    for(i in 1:n){

      training_points <- x[-i,,drop=FALSE]
      test_point <- x[i,,drop=FALSE]
      training_runs <- fx[-i,]

      # G - model matrix.
      G_train <- G[-i,,drop=FALSE]
      g_test <- G[i,,drop=FALSE]
      # Correlation matrix
      C_train <- C[-i,-i]
      c_test <- C[i,-i,drop=FALSE]
  ...
  cl_optimisation <- stats::optim( log_init, LOOCV_accuracy, method = method, lower = loglower, upper = logupper )
